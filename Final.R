...
th <- st_read("Raw_2019_Trips/district_of_columbia_network_volumes.shp")
...
roads_cleaned %>%
  ...
  st_write("Consolidated_2019_Trips/2019_Thurs_Consolidated_withnames.shp", delete_layer = TRUE)


# Read in Thursday 2022 trips
th <- st_read("Raw_2022_Trips/district_of_columbia_network_volumes.shp")
# get just the sidewalks and save as an object
sidewalks <- th %>%
  filter(lanes == 0)
# get just the roads and save as an object
...
roads_cleaned %>%
  transmute(stableEdge,
            ...
            total = total_trip + total_trip_sidewalks,
            direction, streetName) %>%
  st_write("Consolidated_2022_Trips/2022_Thurs_Consolidated.shp", delete_layer = TRUE)


# Filtering and cleaning data ---------------------------------------------
#Read 2019 data and remove all segments with NA or 0 as street name.
st_read("Intermediate_Final_2019/Final_2019_Data.shp") %>%
  filter(!is.na(streetName) & streetName != 0) %>%
  st_write("Final_2019.shp", delete_layer = TRUE)

#Read 2022 data and remove all segments with NA or 0 as street name.
st_read("Intermediate_Final_2022/Final_2022_Data.shp") %>%
  filter(!is.na(streetName) & streetName != 0) %>%
  st_write("Final_2022.shp", delete_layer = TRUE)

#Bind into a finalized dataset and add yearbinary and treatment
final19 <- st_read("Final_2019.shp") %>%
  select(!"Shape_Leng")
final22 <- st_read("Final_2022.shp")
...
#Filtering for roads that match similar biking characteristics in the study corridor in the before period. (Option to add additional conditions here.) Calculating mode share.
selectingstreets <- final_all_data %>%
  group_by(streetName) %>%
  filter(yearbinary==0 & mean(biking)>100 & mean(biking)<400 & mean(total)>4000 & mean(total)<9000) %>%
  distinct(streetName)%>%
  pull()
selectingstreets_manual <- final_all_data %>%
  group_by(streetName) %>%
  filter(streetName == "17th Street Northwest" | streetName =="P Street Northwest" | streetName == "6th Street Northeast" | streetName =="7th Street Northwest" | streetName =="22nd Street Northwest" | streetName =="W Street Northwest" | streetName =="V Street Northwest" | streetName =="E Street Northwest" | streetName =="C Street Northeast" | streetName =="K Street Northwest" | streetName =="K Street Northeast" | streetName =="15th Street Northeast") %>%
  distinct(streetName) %>%
  pull()
...
#alternative dataset that is consolidated by streetname
merged_data <- final_filtered_data %>%
  st_drop_geometry() %>%
  group_by(yearbinary, treatment, streetName) %>%
  summarize(biking=mean(biking), walking=mean(walking), auto=mean(priv_auto+carpool+on_demand), bikeshare=mean(bikeshare), walkshare=mean(walkshare), autoshare=mean(autoshare))
...
final_filtered_data_nostudyarea %>%
  mapview()
merged_data_nostudyarea <- merged_data %>%
  filter(treatment != 2)%>%
  pivot_longer(cols = c(bikeshare, autoshare)) %>%
  transmute(yearbinary, 
            treatment = ifelse(treatment == 1, 1, 0),
            mode = ifelse(name == "bikeshare", 1, 0),
            value_log =log(value))

filtered_data_studyarea <- final_all_data %>%
  mutate(bikeshare = biking/total*100, autoshare = (priv_auto+on_demand+carpool)/total*100, walkshare=walking/total*100) %>%
  filter(bikeshare>0 & autoshare>0 & walkshare>0 & autoshare<80 & bikeshare >2 & bikeshare < 25)
...
did_bikeshare2 <- lm(value_log ~ treatment * yearbinary * mode, data = merged_data_nostudyarea)
summary(did_bikeshare2)


#ADDITIONAL TESTS#
VStreet <- final_filtered_data_nostudyarea %>%
  filter((treatment == 0 & streetName == "V Street Northwest") | treatment == 1)
did_bikeshare <- lm(value_log ~ treatment * yearbinary * mode, data = VStreet)
summary(did_bikeshare)

Street17th <- final_filtered_data_nostudyarea %>%
  filter((treatment == 0 & streetName == "17th Street Northwest") | treatment == 1)
did_bikeshare <- lm(value_log ~ treatment * yearbinary * mode, data = Street17th)
summary(did_bikeshare)

EStreet <- final_filtered_data_nostudyarea %>%
  filter((treatment == 0 & streetName == "E Street Northwest") | treatment == 1)
did_bikeshare <- lm(value_log ~ treatment * yearbinary * mode, data = EStreet)
summary(did_bikeshare)
...
# Crash Analysis: Difference in difference --------------------------------
#Identifying mean biking statistics in study corridor in the before period
all_crash %>%
...
crash_filtered_final <- crash_filtered %>%
  filter(treatment != 2)
...
#ADDITIONAL TESTS#
VStreetcrash <- all_crash %>%
  filter((treatment == 0 & streetName == "V Street Northwest") | treatment == 1)
did_totalcrashes <- lm(totalcrashes_per_trip ~ treatment * period, data = VStreetcrash)
summary(did_totalcrashes)
did_bikecrashes <- lm(bikecrashes_per_trip ~ treatment * period, data = VStreetcrash)
summary(did_bikecrashes)
did_autocrashes <- lm(autocrashes_per_trip ~ treatment * period, data = VStreetcrash)
summary(did_autocrashes)

Street17thcrash <- all_crash %>%
  filter((treatment == 0 & streetName == "17th Street Northwest") | treatment == 1)
did_totalcrashes <- lm(totalcrashes_per_trip ~ treatment * period, data = Street17thcrash)
summary(did_totalcrashes)
did_bikecrashes <- lm(bikecrashes_per_trip ~ treatment * period, data = Street17thcrash)
summary(did_bikecrashes)
did_autocrashes <- lm(autocrashes_per_trip ~ treatment * period, data = Street17thcrash)
summary(did_autocrashes)

EStreetcrash <- all_crash %>%
  filter((treatment == 0 & streetName == "E Street Northwest") | treatment == 1)
did_totalcrashes <- lm(totalcrashes_per_trip ~ treatment * period, data = EStreetcrash)
summary(did_totalcrashes)
did_bikecrashes <- lm(bikecrashes_per_trip ~ treatment * period, data = EStreetcrash)
summary(did_bikecrashes)
did_autocrashes <- lm(autocrashes_per_trip ~ treatment * period, data = EStreetcrash)
summary(did_autocrashes)

RStreetcrash <- all_crash %>%
  filter((treatment == 0 & streetName == "R Street Northwest") | treatment == 1)
did_totalcrashes <- lm(totalcrashes_per_trip ~ treatment * period, data = RStreetcrash)
summary(did_totalcrashes)
did_bikecrashes <- lm(bikecrashes_per_trip ~ treatment * period, data = RStreetcrash)
summary(did_bikecrashes)
did_autocrashes <- lm(autocrashes_per_trip ~ treatment * period, data = RStreetcrash)
summary(did_autocrashes)